function [VecPInv, VecPBest, EVecBest] = MultiNelderMead(Coord2Dl, Coord2Dp, Matrix0, VecP0)
% Размерность задачи
N = 6;
...
% Маскимальные смещения начальной точки
Tmax = 10;
Rmax = 2*pi/180;

% Инициализация числа начальных точек и других параметров
MN = N*2;
VecP1 = VecP0;
VecPBest = zeros(N, MN);
EVecBest = zeros(MN,1);
...
for m = 1:MN
    ...
    % Рандомное смещение центральной точки с амплитудой половины
    % допустимого диапазона
    if (m > 1)
        for j = 1:3
            VecP1(j) = VecP0(j) + (2*rand(1) - 1)*Tmax/(2);
        end
        
        for j = 4:6
            VecP1(j) = VecP0(j) + (2*rand(1) - 1)*Rmax/(2);
        end
    end
    ...
    for i = 2:(N+1)
        for j = 1:3
            if (i ~= j)
                VecP(j,i) = VecP1(j) + d1T;
            else
                VecP(j,i) = VecP1(j) + d2T;
            end
        end

        for j = 4:6
            if (i ~= j)
                VecP(j,i) = VecP1(j) + d1R;
            else
                VecP(j,i) = VecP1(j) + d2R;
            end
        end
    end
    %--------------------------------------------------------------------------
    ...
    % Запись в вектор лучших векторов параметров и величин ошибок
    [~,l] = min(EVec);
    VecPBest(:,m) = VecP(:,l);
    EVecBest(m) = EVec(l);
end

% На вывод лучшего вектора парметров и ошибки
[~,tmin] = min(EVecBest);
VecPInv = VecPBest(:,tmin);
